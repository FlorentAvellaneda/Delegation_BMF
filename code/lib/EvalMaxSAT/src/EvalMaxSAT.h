...
public:
    ...
    void atMostKOnSoftVar(std::vector<int> clause, unsigned int am) {

        while(clause.size() > am) {
            auto saveClause = clause;
            t_weight w = _weight[ abs(clause[0]) ];
            ...
            t_weight k = saveClause.size()-am;
            MonPrint("AM1: cost = ", cost, " + ", w * k);
            cost += w * k;

            assert(saveClause.size() > am);

            //newAssum.push_back( addWeightedClause(saveClause, w) );

            std::vector<int> L;
            for(auto l: saveClause) {
                L.push_back(-l);
            }

            std::shared_ptr<VirtualCard> card = std::make_shared<CardIncremental_Lazy>(this, L, k);
            save_card.push_back( {card, k, w} );
