...
parser.add_argument('--k', nargs='?', help='Rank', type=int, default=9999999)

parser.add_argument('--t', nargs='?', help='timeout', type=float, default=10800)
...
# Number of one in X
nb_ones = int(np.nansum(X))

kInit = min(args.k, X.shape[0]-1, X.shape[1]-1, nb_ones-1)

t=time.time()-time1
print("Result CG on ", os.path.basename(dataset_path) ," with k = ",kInit+1,": 0 errors in ", t, " sec")
...
for k in range(kInit, 0, -1):
    ...
    if np.nansum(np.abs(X - boolean_matrix_product(bmf.A, bmf.B))) != 0:
        ...
        print("Best for ",os.path.basename(dataset_path)," k = ", k+1, " in ", (time.time()-time1)/60.0, " min with dual bound = ", bmf.best_dual_bound)
        break
