...
class EvalMaxSAT : public VirtualMAXSAT {
    ...
    void reduceCliqueV2(std::list<int> & clique) {
        std::cout << "reduce " << clique.size() << std::endl;
...
public:
    ...
    virtual void atMostKOnSoftVar(const std::vector<int> &clause, unsigned int k) {
        //std::cout << "atMostKOnSoftVar(" << clause << ")" << std::endl;
        assert([&](){

            for(auto lit: clause) {
                unsigned int var = static_cast<unsigned int>(abs(lit));
                if(_weight[var] == 0) {
                    std::cout << "_weight["<<var<<"]" << " = " << _weight[var] << std::endl;
                    return false;
                }
            }
...
public:
    ...
    std::vector<int> searchFirstConflict() {
        std::vector<int> assumption;
        ...
        auto conflictFound = (solver->solve(assumption) == false);
        ...
        fastMinimize(solver, conflictMin);

        return std::vector<int>(conflictMin.begin(), conflictMin.end());
    }

    bool solve(int valMax=std::numeric_limits<int>::max()) {
